...
对应实验是 C. Capacity and Adjustment Ablation。当前主文不需要做大型 allocation-baseline benchmark，而是用 compact mechanism ablation 验证 capacity 和 QP 的 intended behavior。

```text
主文对比：equal split vs proposed。

机制 case：
1. no unnecessary cooperation when Delta_i = 0；
2. symmetric allocation under symmetric margins；
3. front-boundary preference when front-side margin is larger；
4. rear-boundary preference when rear-side margin is larger。
```

验证重点：

```text
space-sensitive weights shift adjustment away from the tighter neighboring-gap side；
adjacent-gap capacity prevents gap development from consuming protected neighboring margins；
the QP gives the minimum weighted quadratic adjustment under the proposed cost, not a globally minimum traffic disturbance.
```
...
这些 optional baselines 只有在后续实验补齐时才进入正文或附录。当前 ICUS 主文只写 compact ablation verifies intended allocation behavior。
...
B. Feasibility Validation 中的 candidate score table 支撑本节。该表展示每个 candidate gap 的 \(C_i^{coop}\)、\(C_i^{ego}\)、\(J_i\) 和最终 \(i^*\)，说明本文是在 proposed score 下选择 best developed gap。

```text
B supports valuation and selected-gap execution.
```

写作时要克制：

```text
可以说：selected gap is best under the proposed disturbance-efficiency score.
不要说：selected gap is globally optimal for real traffic.
不要把 B 写成大型 selection-baseline 实验。
```
...
B. Feasibility Validation 支撑本节。它展示 selected developed gap 如何转化为 \(d_i\)、\(t_m\)、\(p_m\) 和 MV trajectory。速度/加速度约束可以通过图或正文一句说明满足，不必把这一节扩展成闭环控制验证。

```text
Feasibility Validation
trajectory plot
terminal time/location
speed and acceleration constraint check
```
...
在每个 candidate gap 的协作能力约束下，空间敏感 QP 给出该 gap 在 proposed weighted quadratic cooperation cost 下的最小调整估计。
...
本文不声明 QP 证明了真实交通中的全局最小扰动。
本文不声明 selected gap 是真实交通全局最优 gap。
```
...
For each developable candidate, the adjustment problem yields the minimum weighted quadratic boundary-vehicle adjustment under the proposed space-sensitive cooperation cost and adjacent-gap capacity constraints.
```
...
| Method 模块 | 对应实验 | 写作边界 |
|---|---|---|
| Problem Formulation | B. Feasibility Validation | 展示 candidate gap evaluation 到 selected-gap trajectory execution 的闭环，不写外部性能优越性。 |
| Adjacent-Gap-Preserving Cooperation Capacity | C. Capacity and Adjustment Ablation | 验证 cooperation capacity follows neighboring-gap margins，不写主线车辆可任意调整。 |
| Space-Sensitive Minimum-Disturbance Adjustment | C. Capacity and Adjustment Ablation | 验证 adjustment shifts away from the tighter side；主文只需 equal split vs proposed 的 compact ablation。 |
| Disturbance-Efficiency Valuation | B. Feasibility Validation | 用 candidate score table 展示 \(C_i^{coop}\)、\(C_i^{ego}\)、\(J_i\)、\(i^*\)，不写真实交通全局最优。 |
| Terminal Time/Location and Trajectory | B. Feasibility Validation | 展示 selected developed gap 可转化为 \(t_m\)、\(p_m\) 和 feasible trajectory，不写闭环控制全覆盖。 |
| Overall low-disturbance effect | D. External Algorithm Comparison | 只用 representative successful-merging cases 的 mainline disturbance metrics 支撑，不把 fixed entry/exit time 或 fixed spacing 当作主性能比较。 |
...
如果篇幅紧，也可以将 II 和 III.A 合并部分叙述，或压缩 III 内部小节标题；但不建议删除 valuation 和 trajectory execution 的连接，否则故事会断在“估计协作代价”而不是“选中并执行 gap”。无论最终小节如何压缩，candidate-level estimate -> scoring -> selected-gap execution 的动作顺序不能丢。
