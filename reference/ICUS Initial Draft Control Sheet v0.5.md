...
当前不是继续判断材料是否足够的阶段，而是进入初稿后的软对齐阶段。论文已经有稳定主线，且 `main.tex` 中 II. Problem Formulation、III. Mainline-Disturbance-Aware Cooperative Gap Development 和 IV. Experiments 已经形成英文初稿。现在需要做的不是重新扩展故事，而是收紧 claim 边界，避免后续 Introduction、Abstract 和 Conclusion 被旧实验口径带偏。
...
IV.D 已采用修正后的 RiosTorres/FIFO 理解：FIFO baseline 的目标是按固定顺序和固定 45 m 间距组织车辆通过合流区，不是像本文方法一样先估计每个 candidate gap 的最小必要协作代价。因此，D 节的主性能比较只应使用主线扰动指标；physical entry/exit time 只用于确认两类方法均完成合流，`p_m`、minimum spacing 和 minimum TTC 只作为 planning/safety context。

### 0.3 v0.5 的含义

`v0.5` 是初稿控制版。本轮对齐后的含义不是最终定稿，而是把已经完成的 II/III/IV 初稿、v3 实验证据链和 claim register 放到同一个护栏内。它锁定以下写作控制项：
...
`v0.5` 仍不是最终规范。它不锁死 III 章的小节压缩方式，也不锁死后续实验微调后的最终图表形式。升级到 `v1` 的条件是：

```text
1. Problem Formulation、Method 和 Experiments 初稿均已稳定；
2. III 章小节压缩方式基本确定；
3. Table I/II/III 的真实列名、数据口径和最终场景组合确定；
4. Fig. 1、Fig. 2 以及可选外部对比图的使用方式确定；
5. citation sentence map 已对应到真实文献 key。
```
...
B 节回答 candidate-gap evaluation 是否能闭环执行到 selected developed gap trajectory；C 节回答 adjacent-gap capacity 和 space-sensitive QP 是否具有预期机制行为；D 节回答在 representative successful-merging cases 中，本文方法相对 FIFO baseline 是否减少主线扰动范围、控制强度和恢复时间。

D 节不把 physical entry time、physical exit time、minimum spacing 或 minimum TTC 作为主性能比较指标。原因是 FIFO baseline 在当前实验包中按固定物理入口和固定 45 m 队列间距组织通行，这些量更多反映 baseline 机制设定，而不是与本文 developed-gap valuation 同构的性能目标。
...
2. 压缩实验解释段落；
3. 删除可选外部扰动图，保留 D 节主线扰动表；
4. 将 merge-placement / spacing context 表压缩成正文一句；
5. 删除不必要的速度/加速度小图，保留 Table I/II/III。
```
...
如果贡献段需要提到外部对比，只能写成 representative successful-merging cases show reduced mainline disturbance。primary representative case 可以额外说明 nearly identical physical merge timing，但不能把 comparable timing 写成所有场景的统一结论。
...
| \(\varepsilon_\delta\) | 小调整量死区阈值 | 不进主文，除非后续需要解释数值上接近 0 的微小调整 |
...
|---|---|
| Case | C1 到 C4 |
| Scenario | 内部追踪可用 S02/S06/S01/S08，正文不必显示 scenario id |
...
当前 formal evidence package 中，front-side larger margin / front-side preference 由 S01 的机制行承担；S07 不作为当前正文证据来源。这个信息只用于内部数据追踪，不应在论文正文中写成“某场景替代某场景”。
...
```text
S01, S02, S03, S06, S08 external comparison；
正文可用 Case A-E 表示，内部保留 scenario id 追踪。
```

目标：

```text
证明在 representative successful-merging cases 中，
OneStep/Proposed 相比 RiosTorres/FIFO 能减少主线扰动范围、控制强度和恢复时间。
其中 primary representative case 可额外支持 nearly identical physical merge timing，
但不能把 comparable timing 写成所有场景的统一结论。
```

主表名称建议：

```text
External Comparison on Mainline Disturbance
```
...
| 列 | 含义 |
|---|---|
| Case | Case A-E，正文不使用 S01/S02 作为学术叙事标签 |
| Method | OneStep/Proposed 或 RiosTorres/FIFO |
| Affected mainline vehicles | 受影响主线车数量，衡量扰动范围 |
| Mainline acceleration energy | 主线加速度能量，衡量控制强度 |
| Recovery time | 主线速度扰动恢复时间，衡量持续时间 |
...
这些量可用于正文解释或 context table。建议写法是：physical entry/exit time 用于确认两类方法均完成物理合流；minimum spacing、minimum TTC 和 \(p_m\) 用于 planning/safety context。它们不与 disturbance metrics 混在同一张性能排序表中。

可选 context table：

| Method | Merge-location treatment | Spacing treatment | Role |
|---|---|---|---|
| OneStep/Proposed | selected-gap \(p_m\) computed from developed gap | resulting minimum spacing after selected-gap execution | planning and safety context |
| RiosTorres/FIFO | fixed physical merge entry \(x=555m\) | fixed 45 m target spacing | baseline setting |

如果页数紧，context table 可压缩成正文一句，不必单独成表。
...
如果使用 time-space comparison figure，caption 必须强调机制差异：OneStep/Proposed executes a selected developed gap with local boundary-vehicle adjustment, whereas FIFO regulates a broader vehicle set to form its fixed-spacing queue。不要把该图解释成 entry/exit timing 优劣比较图。
...
```text
Unweighted QP 或更多 allocation baselines 只有在后续实验补齐时才进入正文或附录。
```
...
```text
在两种方法均完成物理合流的代表性成功场景中，
OneStep/Proposed 是否能减少主线扰动范围、控制强度和恢复时间？
```
...
```text
primary representative case 必须保留；
其余有效场景按数据质量和机制覆盖决定是否全部进入主表；
...
physical merge entry time；
physical merge exit time；
selected-gap merging location \(p_m\)；
minimum spacing；
minimum TTC。
```

这些量不进入 D 节主性能表。physical merge entry/exit time 只用于确认两类方法均完成合流；primary representative case 可用它们辅助说明 nearly identical physical merge timing。minimum spacing、minimum TTC 和 \(p_m\) 只作 planning/safety context，不能写成本文方法在这些固定指标上逐项优于 FIFO。
...
不要说 always faster；
不要说 always comparable physical merge timing；
不要把 entry/exit/min spacing/min TTC 写成主性能比较指标。
```
...
| C2 | 边界车辆协作能力由相邻 gap 的剩余安全余量限制。 | 贡献 2 | C 的 S02/S06/S01/S08 | “cooperation capacity is bounded by neighboring-gap margins” | “主线车辆可任意调整” |
...
| C6 | 在代表性成功合流场景中，OneStep/Proposed 相比 FIFO 降低主线扰动范围、控制强度和恢复时间；primary representative case 额外支持相近 physical merge timing。 | 贡献 3 的实验验证 | D 的 external comparison | “in representative successful-merging cases, the proposed method reduces mainline disturbance relative to the FIFO baseline” | “所有场景都优于 FIFO”；“always faster”；“always comparable physical merge timing”；“globally optimal traffic disturbance” |
...
| baseline | FIFO/RiosTorres 类方法提供固定顺序或队列式 CAV merging coordination baseline。 | Rios-Torres / Malikopoulos 或相关 FIFO merging baseline | 后续确认，只支撑 baseline 类别和机制定位，不支撑本文实验结果 |
...
```text
当前优先级是完成 reference docs 的 post-IV soft alignment；
...
```text
1. Problem Formulation（已形成初稿）
2. Method skeleton（已形成初稿）
3. Experiments prose（已形成初稿）
4. reference docs soft alignment（当前任务）
5. Figure 1 方法图与实验图表最终取舍
6. citation support bank + compact Related Work
7. Introduction 正文初稿
8. Abstract
9. Conclusion
```
...
| R9 | Experiments C | 验证 capacity/QP 机制行为。 | S02/S06/S01/S08 | 不写成完整系统消融。 |
| R10 | Experiments D | 比较 OneStep 与 FIFO 的主线扰动。 | S01/S02/S03/S06/S08 representative successful cases | 所有结论限定在 representative successful scenarios；primary case 才可写 comparable physical timing。 |
...
```text
完成 reference docs soft alignment；
随后进入 citation support bank、compact Related Work 和 Introduction 初稿。
```
...
不把可达性当贡献；
不把 C 节机制实验写成完整系统消融；
不把 D 节写成所有场景 physical timing comparable；
所有公式仍按 Method Formula Depth 控制正文深度。
```
...
3. C 节 S02/S06/S01/S08 机制表；
4. D 节 S01/S02/S03/S06/S08 外部对比原始结果。
```

D 节当前主文可使用五个 representative successful-merging cases；后续如果实验继续调整，按“数据有效、机制覆盖、低扰动结论清楚”的标准更新，不按结果漂亮程度筛选。
...
```text
Problem Formulation、Method、Experiments 初稿稳定；
III 章小节压缩方式稳定；
Table I/II/III 的真实列名、数据格式和 D 节场景组合确定；
Fig. 1 和 Fig. 2 的图形设计确定；
四份核心 reference docs 完成 post-IV soft alignment；
